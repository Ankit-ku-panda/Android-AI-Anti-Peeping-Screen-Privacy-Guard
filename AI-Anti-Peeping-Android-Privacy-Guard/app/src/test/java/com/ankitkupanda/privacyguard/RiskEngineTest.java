...
public final class RiskEngineTest {
    ...
    @Test
    public void unknownSingleFaceTriggersShield() {
        RiskEngine engine = engine();
        assertFalse(engine.updateAuthorization(1_000L, 1, false, true).protectedScreen);
        RiskEngine.Decision decision = engine.updateAuthorization(1_800L, 1, false, true);
        assertTrue(decision.protectedScreen);
        assertEquals(RiskEngine.UNKNOWN_FACE_REASON, decision.reason);
    }

    @Test
    public void authorizedSingleFaceStaysClear() {
        RiskEngine engine = engine();
        assertFalse(engine.updateAuthorization(1_000L, 1, true, true).protectedScreen);
        assertFalse(engine.updateAuthorization(5_000L, 1, true, true).protectedScreen);
    }
